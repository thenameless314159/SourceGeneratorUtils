...
public class CSharpSourceGeneratorTests
{
    ...
    [Fact]
    public void Ctor_ShouldCreateInstanceWithDefaultOptionsAndNoBlockWriters()
    {
        var generator = new CSharpSourceGenerator<DefaultTypeSpec>();
        Equal(SourceFileGenOptions.Default, generator._options);
        Empty(generator._blockWriters);
    }

    [Fact]
    public void Create_ShouldAddUsedBlockWritersToGenerator()
    {
        var blockGenerator = new MethodGenerator("TestMethod");
        var generator = CSharpSourceGenerator
            .Create<DefaultTypeSpec>(Options, builder =>
                builder
                    .Use<TestBlockGenerator>()
                    .Use(blockGenerator));

        IsType<CSharpSourceGenerator<DefaultTypeSpec>>(generator);
        var gen = (CSharpSourceGenerator<DefaultTypeSpec>)generator;
        Equal(2, gen._blockWriters.Count);
    }

    [Fact]
    public void GenerateSource_ShouldReturnValidSourceFileDescriptor()
    {
        var generator = new CSharpSourceGenerator<DefaultTypeSpec>(Options);
    ...
    public void GenerateSource_ShouldUseWriterFactoryIfSpecified()
    {
        ...
        var generator = new CSharpSourceGenerator<DefaultTypeSpec>(Options with
        {
    ...
    public void GenerateSource_ShouldWriteSourceBlocks()
    {
        ...
        var generator = new CSharpSourceGenerator<DefaultTypeSpec>(Options, blockGenerators);
    ...
    private sealed class MethodGenerator : CSharpSourceBlockWriter<DefaultTypeSpec>
    {
        private readonly string _methodName;
        public MethodGenerator(string methodName) => _methodName = methodName;

        public override void WriteTo(SourceWriter writer, DefaultTypeSpec target, SourceFileGenOptions options)
        {
    ...
    private sealed class TestBlockGenerator : CSharpSourceBlockWriter<DefaultTypeSpec>
    {
        ...
        public override IEnumerable<string> GetImplementedInterfaces(DefaultTypeSpec target, SourceFileGenOptions options)
            => ImplementedInterfaces;

        public override IEnumerable<string> GetImportedNamespaces(DefaultTypeSpec target, SourceFileGenOptions options)
            => ImportedNamespaces;
